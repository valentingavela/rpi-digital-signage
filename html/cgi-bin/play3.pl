...
my $s = qq
	{
	<script>
	window.setTimeout("location=('/cgi-bin/play3.pl');",$miliseconds );
	</script>
	};
...
sub intervalTpt
{
	...
	if ((lc($media) =~ /mov$/) || (lc($media) =~ /mp4$/))
	{
		#si es video no usar un template sino
		$tptpth = "/var/www/html/templates/inmobiliarias/ds/logo/video_omxplayer-T.html" ;
	}
	else
	{
